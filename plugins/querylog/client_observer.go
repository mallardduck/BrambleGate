...
// clientObserver, if set, is called with every observed client IP — the
...
// querylog ... clientnames subscribes to that stream"). querylog's module
...
var clientObserver atomic.Pointer[func(ip string)]
...
// nil-safe no-op.
func SetClientObserver(f func(ip string)) {
	if f == nil {
...
// observeClient calls the current observer, if any, with ip. No-op when ip
// is empty or no observer is set.
func observeClient(ip string) {
	if ip == "" {
		return
	}
